...
<script>
	...
	function sendCheck() {
		...
		f.method = "post";
		f.action = "<%=cp %>/escp/postWrite_ok.do";
		f.submit();
	}
...
<!-- 						<li style="margin-right: 75px;"><a href="https://www.roomescape.co.kr/board/list.php?board=boardpan"> 보드판 갤러리</a></li>						
				...
				<div class="board_panel">
					<form id="boardFrm" class="boardFrm" method="post" name = "myForm" enctype="multipart/form-data">			
					<!-- 세션 로그인으로 수정해야함 -->
						<input type="hidden" name= "id" value = "${sessionScope.CustomInfo.id }"/>							
							<input type="text" class="input input_text" id="subject" name="bo_subject" placeholder="제목" />
						...
						<style>
			  			...
			  			<textarea rows="12" cols="63" name="bo_content" style="margin: 0px; width: 989px; height: 264px;"></textarea>
						<div>
							
						</div>
			  			<div class="boardFrm_btn">
			  				<input type = "button" style="background-color: #EAEAEA; margin-top: 10px;" class="boardFrm_submit" style="margin-top: 15px;" value="게시글 등록" onclick="sendCheck();">			  				
			  			</div>
					</form>
				</div>
			</div>
